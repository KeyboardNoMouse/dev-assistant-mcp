...
function formatValidation(vr: NonNullable<Workflow["validationResult"]>): string {
  const icon = vr.passed ? "✅" : "❌";
  const lines = [`\nValidation: ${icon} ${vr.summary}`];
  for (const v of vr.validators) {
    const vIcon = v.passed ? "✅" : "❌";
    lines.push(`  ${vIcon} ${v.kind} (${(v.durationMs / 1000).toFixed(1)}s)`);
    if (!v.passed) {
      lines.push(`      ${v.output.split("\n").slice(0, 6).join("\n      ")}`);
    }
  }
  return lines.join("\n");
}
...
function formatWorkflow(wf: Workflow): string {
  ...
  const validationLines = wf.validationResult ? [formatValidation(wf.validationResult)] : [];

  return [...header, ...stepLines, ...validationLines].join("\n");
}
